...
class KDTree
{
  public:
    ...
    bool smallerRadius(const Point<Dim>& target, const Point<Dim>& currentBest,
     ...
     */
    Point<Dim> findNearestNeighbor(const Point<Dim>& target) const;
    Point<Dim> findNearestNeighbor(const Point<Dim>& target,
  ...
  private:
    ...
    void sortPoints(int startIndex, int endIndex, int dimension);

    int pivot(int p, int r, int dimension);

    void selectPivot(int startIndex, int endIndex, int k, int dimension);

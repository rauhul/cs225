...
gradientColorPicker::gradientColorPicker(RGBAPixel fadeColor1,
    ...
    _fadeColor1 = fadeColor1;
    _fadeColor2 = fadeColor2;
    _radius  = radius;
    _centerX = centerX;
    _centerY = centerY;
}
...
RGBAPixel gradientColorPicker::operator()(int x, int y) {
    int d = abs(_centerX - x) + abs(_centerY - y);
    if (d < _radius) {
        RGBAPixel color;
        color.red   = _fadeColor1.red   - floor((d * _fadeColor1.red)   / _radius) + floor((d * _fadeColor2.red)   / _radius);
        color.green = _fadeColor1.green - floor((d * _fadeColor1.green) / _radius) + floor((d * _fadeColor2.green) / _radius);
        color.blue  = _fadeColor1.blue  - floor((d * _fadeColor1.blue)  / _radius) + floor((d * _fadeColor2.blue)  / _radius);
        color.alpha = _fadeColor1.alpha - floor((d * _fadeColor1.alpha) / _radius) + floor((d * _fadeColor2.alpha) / _radius);
        return color;
    } else {
        return _fadeColor2;
    }

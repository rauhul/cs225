...
PNG brighten(PNG original, int amount) {
    for (size_t y = 0; y < original.height(); y++) {
        for (size_t x = 0; x < original.width(); x++) {
            original(x,y)->red   = original(x,y)->red   + amount >= 255 ? 255 : original(x,y)->red   + amount;
            original(x,y)->green = original(x,y)->green + amount >= 255 ? 255 : original(x,y)->green + amount;
            original(x,y)->blue  = original(x,y)->blue  + amount >= 255 ? 255 : original(x,y)->blue  + amount;
        }
    }
    return original;
}
...
PNG blendImages(PNG firstImage, PNG secondImage) {
    PNG blentImage(firstImage.width(),firstImage.height());
    for (size_t y = 0; y < firstImage.height(); y++) {
        for (size_t x = 0; x < firstImage.width(); x++) {
            blentImage(x,y)->red   = (firstImage(x,y)->red   + secondImage(x,y)->red)/2;
            blentImage(x,y)->green = (firstImage(x,y)->green + secondImage(x,y)->green)/2;
            blentImage(x,y)->blue  = (firstImage(x,y)->blue  + secondImage(x,y)->blue)/2;
            blentImage(x,y)->alpha = (firstImage(x,y)->alpha + secondImage(x,y)->alpha)/2;
        }
    }
    return blentImage;
}
